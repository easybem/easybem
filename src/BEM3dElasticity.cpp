...
extern double *qPoints2, *qWeights2;
extern double *qPoints3, *qWeights3;
extern double *qPoints4, *qWeights4;
extern int quadn;
extern double *qPoints, *qWeights;
extern int quadTrin;
extern Point2D *qTriPoints;
extern double *qTriWeights;
...
void SingleLayerElasticity3d_idPanels (double *us, double *vs, double &I,
...
void SingleLayerElasticity3d_commonEdge (double *us, double *vs, double *ws,
...
void SingleLayerElasticity3d_commonVertex (double *us, double *vs,
...
void SingleLayerElasticity3d_disjointPanels
...
void zeroe (double *&matrix, int m, int n)
{
...
void getElasticity3d (int np, const double *points,
                      int ne, const int *elemNodes,
                      double nu, // E = 1.0
                      double *&V, double *&K, double *&D, double *&M,
                      int order=4)
{
  int i, j, k, l, cmnIdxi[2],cmnIdxj[2],cmnIdxSize, remIdxi, remIdxj, tmp;
  int idx, ne2 = 2*ne, ne3 = 3*ne, np2 = 2*np, np3 = 3*np;
  ...
  V = new double[ne3*ne3];
  zeroe(V,ne3,ne3);
  K = new double[ne3*np3];
  zeroe(K,ne3,np3);
  M = new double[ne3*np3];
  zeroe(M,ne3,np3);
  D = new double[np3*np3];
  zeroe(D,np3,np3);

  int progress = 0;
  std::cout << "BEM 3d elasticity assembling - " << np << " nodes, "
  ...
  for (i=0; i<ne; i++)
    {
      for (k=0; k<3; k++)
        faceNodesi[k] = (int)(elemNodes[3*i+k]);
      for (k=0; k<3; k++)
      ...
      for (j=0; j<ne; j++)
        {
          for (k=0; k<3; k++)
            faceNodesj[k] = (int)(elemNodes[3*j+k]);
      ...
      progress = (100*i)/ne;
      std::cout << "\rBEM 3d elasticity assembling - " << np << " nodes, "
                << ne << " elements : " << progress << "%" << std::flush;
     }
  std::cout << "\rBEM 3d elasticity assembling - " << np << " nodes, "

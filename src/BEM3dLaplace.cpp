...
double SingleLayerLaplace3d_commonVertex (double *us, double *vs,
					  double *ws, double *zs)
{
  int i, j, k;
...
double SingleLayerLaplace3d_disjointPanels
...
void zero (double *&matrix, int m, int n)
{
                   ...
                   int order=4)
{
  ...
  int progress = 0;
  std::cout << "BEM 3d Laplace assembling - " << np << " nodes, "
      ...
      for (j=0; j<ne; j++, idx++)
        {
          ...
          for (k=0, cmnIdxSize=0; k<3; k++)
            {
              l = Find<int>(faceNodesj[k],faceNodesi,3); //2);
              if (l>0)
                {
...
void evaluateLaplace3d (const double *nodes,
                        int ne, const int *elemNodes,
                        int np, const double *points,
                        const double *t, const double *u,
                        double *&Vt_Wu,
                        int order=4)
{
  int i, j, k, faceNodesj[3];
